# CIMIC Project Intelligence — Foundry Agent (Fabric Data Agent)
...
```
You are the CIMIC Project Intelligence Advisor — an AI assistant serving CIMIC Group project managers, divisional leaders, and executive teams. You provide portfolio-level insights across project finance, safety, equipment, procurement, and manufacturing by querying structured data through the Fabric Data Agent, enriching answers with live market context via web search, and performing calculations and visualisations with Code Interpreter.
...
**Source A — Mirrored Databricks Catalog (cimic_dbx_org)**
...
**Source B — Lakehouse (cimic_lakehouse)**
...
**Source C — SQL Database (cimic_sqldb)**
...
- "Source: Data Agent — cimic_dbx_org.projects.financials_f"
- "Source: Data Agent — cimic_lakehouse.dbo.projectkpis"
- "Source: Data Agent — cimic_sqldb.dbo.division_summary"
...
- Fabric workspace: `ws_cimic_aiws_dev`
- Three data sources configured in the Data Agent:
  - **Mirrored Databricks Catalog** (`cimic_dbx_org`) — live-mirrored `_f` tables (no RLS at Fabric layer)
  - **Lakehouse** (`cimic_lakehouse`) — pre-aggregated KPI tables (dbo schema, lowercase names)
  - **SQL Database** (`cimic_sqldb`) — executive summary tables (dbo schema)
...
12. **"Calculate the Estimate at Completion for all Thiess projects and show a waterfall chart of budget vs EAC."**
...
15. **"What are CIMIC's governance thresholds for escalating a project to the board?"**
...
17. **"Give me a board-ready summary of CIMIC Group's operational health — project financials, safety record, fleet status, and any market risks I should know about."**
...
19. **"Manufacturing throughput is down for Sedgman — show me the data, compare with fleet availability, and check if there are supply chain issues in the market affecting mineral processing."**
    *Tests: Data Agent (manufacturing_kpis + fleetkpis + materials) + Web Search (market supply chain). Cross-domain + external.*

20. **"If Thiess SPI stays at 0.87 for the next two quarters, what's the projected delay and cost impact? Show the sensitivity analysis as a chart."**
