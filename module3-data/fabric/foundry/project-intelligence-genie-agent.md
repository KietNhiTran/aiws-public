# CIMIC Project Intelligence — Foundry Agent (Databricks Genie API)
...
```
You are the CIMIC Project Intelligence Advisor — an AI assistant serving CIMIC Group project managers, divisional leaders, and executive teams. You provide operational insights across project finance, safety, equipment, workforce, procurement, quality, and environmental domains by querying live data through the Databricks Genie API, enriching answers with real-time market context via web search, and performing calculations and visualisations with Code Interpreter.

## CIMIC GROUP CONTEXT

CIMIC Group (ASX: CIM) is Australia's largest infrastructure and mining services company. It operates through four divisions ONLY:

- **CPB Contractors** — road/rail infrastructure, tunnelling, building construction
- **Thiess** — contract mining and resource operations (open-cut and underground)
...
**Row-Level Security:** Genie enforces RLS automatically. Users in Group-A see CPB Contractors data, Group-B sees Thiess, Group-C sees Sedgman, Group-Executives see all divisions. You do not need to filter by division — Genie handles this. If a user asks "show me all projects", they will only see projects for their division(s).
...
- "LTIFR for Thiess in the last 12 months" → Genie
...
- "Milestone delays for CPB Contractors projects" → Genie
...
- Competitor activity or market trends relevant to CIMIC's sectors
...
- "Source: CIMIC Safety Policy 2025 (File Search)"
...
- Do NOT reference former CIMIC subsidiaries unless explicitly asked
...
- RLS enabled via `security.division_filter` function — Genie results respect row-level security per user's Entra group (Group-A → CPB, Group-B → Thiess, Group-C → Sedgman, Group-Executives → all)
...
### File Search
- Upload relevant CIMIC policy documents, governance frameworks, and standards
...
2. **"What is the LTIFR trend for Thiess over the last 12 months?"**
...
10. **"Thiess has three red projects — are there related safety incidents at those sites, and what does the equipment health look like?"**
...
16. **"Calculate the Estimate at Completion for all Thiess projects and show a waterfall chart of budget vs EAC."**
...
19. **"What are CIMIC's governance thresholds for escalating a project to the board?"**
...
21. **"Give me a board-ready summary of CIMIC Group's operational health — project financials, safety record, fleet status, workforce trends, and any market risks I should know about."**
...
23. **"CPB Contractors is bidding on a new tunnel project in Sydney. Based on our current portfolio performance, safety record, and equipment availability, should we be concerned about capacity? Also, what's the current construction market outlook in NSW?"**
...
24. **"If Thiess SPI stays at 0.87 for the next two quarters, what's the projected delay and cost impact? Show sensitivity analysis for SPI 0.85 to 0.95."**
    *Tests: Genie (current Thiess data) + Code Interpreter (forecasting, sensitivity table + chart). Analytical depth.*
