...
> - **Lakehouse:** `cimic_lakehouse` — tables: ProjectKPIs, SafetyKPIs, FleetKPIs
> - **SQL Database:** `cimic_sqldb` — tables: division_summary, monthly_kpis, manufacturing_kpis, supplier_scorecard
>
...
```
You are a cross-domain operations intelligence assistant for CIMIC Group, Australia's largest infrastructure and mining services company. You provide portfolio-level insights by aggregating data from multiple sources — mirrored databases, lakehouses, and SQL databases — covering project finance, safety, equipment, and procurement into concise, decision-ready summaries.
...
CIMIC GROUP CONTEXT:
CIMIC Group (ASX: CIM) is Australia's largest infrastructure and mining services company. It operates through four divisions:
- CPB Contractors — Australia's leading infrastructure contractor (roads, rail, tunnels, buildings)
- Thiess — World's largest mining services provider (open-cut and underground mining)
- Sedgman — Mineral processing and project delivery for the resources sector
- Pacific Partnerships — Development and management of public-private partnerships (PPP/concessions)
...
2. cimic_lakehouse (Lakehouse) — Use for pre-aggregated KPI tables (ProjectKPIs, SafetyKPIs, FleetKPIs). Best for divisional trending, period-over-period comparisons, and dashboard-style summaries.
3. cimic_sqldb (SQL Database) — Use for curated executive summary tables (division_summary, monthly_kpis, supplier_scorecard). Best for board-ready snapshots and cross-functional summaries.
...
PURPOSE: Granular row-level operational data across all CIMIC domains. These are unfiltered _f table copies (no RLS applied at the Fabric layer). Use this source for detailed drilldowns, individual record lookups, cross-domain joins, and ad-hoc analysis that requires row-level granularity.

TABLE: financials_f
Schema: cimic_dbx_org.projects.financials_f
...
Schema: cimic_dbx_org.equipment.equipment_telemetry_f
...
TABLE: incidents_f
Schema: cimic_dbx_org.safety.incidents_f
...
TABLE: materials
Schema: cimic_dbx_org.procurement.materials
...
### Source: cimic_lakehouse (Lakehouse) — Instructions

```
LAKEHOUSE: cimic_lakehouse
Pre-aggregated KPI tables derived from CIMIC Group's mirrored Databricks data. One row per division (or per division per month). Use for fast executive summaries and divisional comparisons without aggregating granular records.

TABLE: projectkpis
Full path: cimic_lakehouse.dbo.projectkpis
Description: Portfolio financial KPIs aggregated by division.
Columns: division (str: CPB Contractors|Thiess|Sedgman|Pacific Partnerships), total_budget (decimal, sum of all project budgets in AUD), total_actual_cost (decimal, sum of actual costs), avg_cpi (decimal, average Cost Performance Index), avg_spi (decimal, average Schedule Performance Index), project_count (int, number of active projects), red_projects (int, number of projects in Red status)
Use for: Division-level financial comparisons, portfolio health dashboards, identifying which division has the most risk.

TABLE: safetykpis
Full path: cimic_lakehouse.dbo.safetykpis
...
TABLE: fleetkpis
Full path: cimic_lakehouse.dbo.fleetkpis
...
### Source: cimic_sqldb (SQL Database) — Instructions
...
Full path: cimic_sqldb.dbo.division_summary
Description: One-row-per-division executive snapshot combining financial, fleet, and safety KPIs.
Columns: division (str: CPB Contractors|Thiess|Sedgman|Pacific Partnerships), total_budget (decimal, AUD), total_actual_cost (decimal, AUD), avg_cpi (decimal), avg_spi (decimal), project_count (int), red_projects (int), fleet_count (int, total equipment), fleet_operational_pct (decimal, % fleet operational), incident_count (int, total incidents), exceedance_count (int, environmental exceedances), last_updated (datetime)
Use for: Executive dashboards, single-glance divisional health, board papers.

TABLE: monthly_kpis
Full path: cimic_sqldb.dbo.monthly_kpis
...
Full path: cimic_sqldb.dbo.manufacturing_kpis
...
Full path: cimic_sqldb.dbo.supplier_scorecard
...
6. "What is the Estimate at Completion for all Thiess projects?" *(mirrored DB)*
...
11. "Give me a CIMIC Group executive dashboard — portfolio performance, safety record, and fleet status across all divisions"
12. "Which division is our biggest risk right now? Consider financial performance, safety incidents, and equipment health"
13. "Compare Thiess vs CPB Contractors across all key metrics — budget, safety, and fleet utilisation"
...
FROM cimic_dbx_org.projects.financials_f f
LEFT JOIN cimic_dbx_org.safety.incidents_f i
  ON f.division = i.division AND i.incident_date >= DATEADD(month, -3, GETDATE())
WHERE f.status = 'red'
  AND f.reporting_period = (SELECT MAX(reporting_period) FROM cimic_dbx_org.projects.financials_f)
GROUP BY f.division
...
LEFT JOIN (
    ...
    FROM cimic_dbx_org.safety.incidents_f
    GROUP BY division
...
WHERE f.reporting_period = (SELECT MAX(reporting_period) FROM cimic_dbx_org.projects.financials_f)
...
FROM cimic_dbx_org.safety.incidents_f i
INNER JOIN cimic_dbx_org.equipment.equipment_telemetry_f e
...
FROM cimic_dbx_org.procurement.materials m
...
FROM cimic_lakehouse.dbo.safetykpis
...
FROM cimic_sqldb.dbo.division_summary
ORDER BY total_budget DESC;
```
...
FROM cimic_sqldb.dbo.supplier_scorecard
...
> **Minimal setup:** Create ONE OneLake Security role on `cimic_dbx_org` to demo RLS with the equipment telemetry table.
...
1. Open `cimic_dbx_org` → **Manage OneLake security**
...
| **Row security** | `SELECT * FROM equipment.equipment_telemetry_f WHERE division = 'CPB Contractors'` |

4. Ensure Vinoth has **Viewer** workspace role (Admins/Members/Contributors bypass OLS)

### Test as Vinoth (Group-A → CPB Contractors only)

**Should return CPB Contractors equipment data only:**

1. "Show me all equipment and their current status"
   → ✅ Only CPB Contractors equipment from `equipment_telemetry_f`

2. "Which equipment has engine temperature above 100°C?"
   → ✅ Only CPB Contractors assets

3. "How many pieces of equipment are in Critical status?"
   → ✅ CPB Contractors count only

4. "List equipment sorted by operating hours — which are overdue for maintenance?"
   → ✅ Only CPB Contractors fleet

**Should return NO data (proves RLS is working):**

5. "Show me Thiess equipment telemetry"
   → ❌ Zero rows — Vinoth cannot see Thiess equipment

6. "Compare equipment health across all divisions"
   → Only CPB Contractors visible (other divisions blocked by OLS)
...
SELECT COUNT(*) FROM cimic_dbx_org.projects.financials_f;
-- Expected: 0
```
