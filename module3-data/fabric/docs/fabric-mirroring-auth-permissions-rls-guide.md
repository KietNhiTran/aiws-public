...
>
> **Context:** CIMIC workshop — mirroring the `cimic` catalog from workspace `adb-7405614861019645.5.azuredatabricks.net` into Fabric.
...
└───────────────────────────────────────────┼───────────┼────────┼────────┘
                    ...
                    │  (cimic catalog)  use groups          + EXTERNAL   │
                    │       │            for RLS             USE SCHEMA  │
                    │       │                                    │        │
                    │       ▼                                    │        │
...
3. Left sidebar → **"App registrations"** → **"+ New registration"**
   ...
   | Field | Value |
   |-------|-------|
   | **Name** | `fabric-databricks-mirror-cimic-dev` |
...
|-------|-------|------------|
| **Application (client) ID** | Overview page | `5319dcfd-60d3-4bce-9d26-7e9a6dd81503` |
| **Directory (tenant) ID** | Overview page | `f7bfcd96-5c56-4a99-8851-b2f678d69f15` |
| **Object ID** | Overview page | `8b66dc18-bd61-4b14-8b3b-364fa6c4aff7` |
...
4. Search for **"AzureDatabricks"** (Application ID: `2ff814a6-3304-4ab8-85cb-cd0e6f879c1d`)
...
4. Select **"Microsoft Entra ID managed"**
   ...
   |-------|-------|
   | **Name** | `fabric-databricks-mirror-cimic-dev` |
   | **Microsoft Entra application ID** | `5319dcfd-60d3-4bce-9d26-7e9a6dd81503` |
...
4. Search for `fabric-databricks-mirror-cimic-dev`
5. Set role: **"User"**
6. Click **"Save"**
...
GRANT USE CATALOG ON CATALOG cimic TO `5319dcfd-60d3-4bce-9d26-7e9a6dd81503`;
GRANT USE SCHEMA ON CATALOG cimic TO `5319dcfd-60d3-4bce-9d26-7e9a6dd81503`;
GRANT SELECT ON CATALOG cimic TO `5319dcfd-60d3-4bce-9d26-7e9a6dd81503`;
```
...
GRANT EXTERNAL USE SCHEMA ON SCHEMA cimic.projects TO `5319dcfd-60d3-4bce-9d26-7e9a6dd81503`;
GRANT EXTERNAL USE SCHEMA ON SCHEMA cimic.equipment TO `5319dcfd-60d3-4bce-9d26-7e9a6dd81503`;
GRANT EXTERNAL USE SCHEMA ON SCHEMA cimic.safety TO `5319dcfd-60d3-4bce-9d26-7e9a6dd81503`;
GRANT EXTERNAL USE SCHEMA ON SCHEMA cimic.procurement TO `5319dcfd-60d3-4bce-9d26-7e9a6dd81503`;
```
...
**Verify grants:**
```sql
SHOW GRANTS ON CATALOG cimic;
SHOW GRANTS ON SCHEMA cimic.projects;
...
Users:                        ──►     Users (auto-synced)
  alice@cimic.com.au          SCIM      alice@cimic.com.au
  bob@cimic.com.au                      bob@cimic.com.au
...
3. Fill in the connection details:
   ...
   | Field | Value |
   |-------|-------|
   | **Connection name** | `databricks-cimic-dev` (or any descriptive name) |
   | **Connection type** | **Azure Databricks** |
   | **Databricks workspace URL** | `https://adb-7405614861019645.5.azuredatabricks.net` |
   | **Authentication kind** | **Service Principal** |
   | **Tenant ID** | `f7bfcd96-5c56-4a99-8851-b2f678d69f15` |
   | **Service Principal client ID** | `5319dcfd-60d3-4bce-9d26-7e9a6dd81503` |
...
5. Now create the mirrored catalog:
   ...
   - In the connection dropdown, select the connection you just created (`databricks-cimic-dev`)
...
3. Configure the **Databricks workspace connection**:
   ...
   | **Authentication kind** | **Service Principal** |
   | **Tenant ID** | `f7bfcd96-5c56-4a99-8851-b2f678d69f15` |
   | **Client ID** | `5319dcfd-60d3-4bce-9d26-7e9a6dd81503` |
...
2. Create a **new ADLS connection**:
   ...
   | Field | Value |
   |-------|-------|
   | **URL** | The ADLS path where your catalog data is stored, e.g. `https://sacimicaiwsauedev.dfs.core.windows.net/manufacturing/aiws-dev` (be specific — folder level, not just storage account) |
...
3. **In the Azure Portal** — grant ADLS access to whichever identity you used:
   ...
   **Option B: Service Principal**
   - In Azure Portal → ADLS storage account → **Access Control (IAM)** → **+ Add role assignment**
   - Assign **Storage Blob Data Reader** to the SP (`5319dcfd-60d3-4bce-9d26-7e9a6dd81503`)
...
> ```
>
> **Which CIMIC tables currently have row filters?**
> - `cimic.projects.financials` — ⚠️ has `division_filter` row filter → **will fail to sync**
> - `cimic.safety.incidents` — ⚠️ has `division_filter` row filter → **will fail to sync**
> - `cimic.equipment.equipment_telemetry` — ✅ no row filter → should mirror fine
> - `cimic.procurement.materials` — ✅ no row filter → should mirror fine
>
> **Fix — Option A: Remove row filters for mirroring (then re-apply later)**
>
> Run in Databricks SQL:
> ```sql
> ALTER TABLE cimic.projects.financials DROP ROW FILTER;
> ALTER TABLE cimic.safety.incidents DROP ROW FILTER;
> ```
>
> After mirroring is established and syncing, you can optionally re-apply the filters for the Genie MCP path:
> ```sql
> ALTER TABLE cimic.projects.financials SET ROW FILTER cimic.security.division_filter ON (division);
> ALTER TABLE cimic.safety.incidents SET ROW FILTER cimic.security.division_filter ON (division);
> ```
...
>
> **This is now automated** in `cimic_setup/04_configure_rls.py` (Section 2b). When `create_mirror_views=yes`, the notebook creates these views before applying row filters:
> ```sql
> CREATE OR REPLACE VIEW cimic.projects.financials_f AS
>   SELECT * FROM cimic.projects.financials;
>
> CREATE OR REPLACE VIEW cimic.safety.incidents_f AS
>   SELECT * FROM cimic.safety.incidents;
> ```
...
> # or with SP grants:
> # Set mirror_sp_id widget to your SP Application ID (e.g. 5319dcfd-60d3-4bce-9d26-7e9a6dd81503)
> ```
> Then in the Fabric mirroring setup, select these tables:
> - `cimic.projects.financials_f` (view, no row filter)
> - `cimic.safety.incidents_f` (view, no row filter)
> - `cimic.equipment.equipment_telemetry` (base table, no row filter)
> - `cimic.procurement.materials` (base table, no row filter)
>
...
|-------------|-------------|------------|----------------------|
| `RoleCPB*` (Finance/Safety/Equipment) | `Group-A` | `SELECT * FROM {table} WHERE division = 'CPB Contractors'` | `IS_ACCOUNT_GROUP_MEMBER('Group-A')` |
| `RoleThiess*` (Finance/Safety/Equipment) | `Group-B` | `SELECT * FROM {table} WHERE division = 'Thiess'` | `IS_ACCOUNT_GROUP_MEMBER('Group-B')` |
| `RoleSedgman*` (Finance/Safety/Equipment) | `Group-C` | `SELECT * FROM {table} WHERE division = 'Sedgman'` | `IS_ACCOUNT_GROUP_MEMBER('Group-C')` |
...
- `cimic.projects.financials_f` (or `financials` if no UC row filter)
- `cimic.equipment.equipment_telemetry`
- `cimic.safety.incidents_f` (or `incidents` if no UC row filter)
- `cimic.procurement.materials` (shared reference data — all roles can read)
...
##### Role 4: `RoleThiessFinance`

| Setting | Value |
|---------|-------|
| **Role name** | `RoleThiessFinance` |
...
**Row security expression:**
```sql
SELECT * FROM projects.financials_f WHERE division = 'Thiess'
```

---

##### Role 5: `RoleThiessSafety`

| Setting | Value |
|---------|-------|
| **Role name** | `RoleThiessSafety` |
| **Assign Entra group** | `Group-B` |
...
**Row security expression:**
```sql
SELECT * FROM safety.incidents_f WHERE division = 'Thiess'
```

---

##### Role 6: `RoleThiessEquipment`

| Setting | Value |
|---------|-------|
| **Role name** | `RoleThiessEquipment` |
| **Assign Entra group** | `Group-B` |
...
**Row security expression:**
```sql
SELECT * FROM equipment.equipment_telemetry_f WHERE division = 'Thiess'
```

---

##### Role 7: `RoleSedgmanFinance`

| Setting | Value |
|---------|-------|
| **Role name** | `RoleSedgmanFinance` |
...
**Row security expression:**
```sql
SELECT * FROM projects.financials_f WHERE division = 'Sedgman'
```

---

##### Role 8: `RoleSedgmanSafety`

| Setting | Value |
|---------|-------|
| **Role name** | `RoleSedgmanSafety` |
| **Assign Entra group** | `Group-C` |
...
**Row security expression:**
```sql
SELECT * FROM safety.incidents_f WHERE division = 'Sedgman'
```

---

##### Role 9: `RoleSedgmanEquipment`

| Setting | Value |
|---------|-------|
| **Role name** | `RoleSedgmanEquipment` |
| **Assign Entra group** | `Group-C` |
...
**Row security expression:**
```sql
SELECT * FROM equipment.equipment_telemetry_f WHERE division = 'Sedgman'
```
...
|---|-----------|-------------|-------|------------------------|
| 1 | `RoleCPBFinance` | `Group-A` | `financials_f` | `SELECT * FROM projects.financials_f WHERE division = 'CPB Contractors'` |
| 2 | `RoleCPBSafety` | `Group-A` | `incidents_f` | `SELECT * FROM safety.incidents_f WHERE division = 'CPB Contractors'` |
| 3 | `RoleCPBEquipment` | `Group-A` | `equipment_telemetry_f` | `SELECT * FROM equipment.equipment_telemetry_f WHERE division = 'CPB Contractors'` |
| 4 | `RoleThiessFinance` | `Group-B` | `financials_f` | `SELECT * FROM projects.financials_f WHERE division = 'Thiess'` |
| 5 | `RoleThiessSafety` | `Group-B` | `incidents_f` | `SELECT * FROM safety.incidents_f WHERE division = 'Thiess'` |
| 6 | `RoleThiessEquipment` | `Group-B` | `equipment_telemetry_f` | `SELECT * FROM equipment.equipment_telemetry_f WHERE division = 'Thiess'` |
| 7 | `RoleSedgmanFinance` | `Group-C` | `financials_f` | `SELECT * FROM projects.financials_f WHERE division = 'Sedgman'` |
| 8 | `RoleSedgmanSafety` | `Group-C` | `incidents_f` | `SELECT * FROM safety.incidents_f WHERE division = 'Sedgman'` |
| 9 | `RoleSedgmanEquipment` | `Group-C` | `equipment_telemetry_f` | `SELECT * FROM equipment.equipment_telemetry_f WHERE division = 'Sedgman'` |
...
|-----------|-------|----------------|
| Alice | `Group-A` | Sees only CPB Contractors rows in financials, incidents, equipment. Sees all procurement. |
| Bob | `Group-B` | Sees only Thiess rows in financials, incidents, equipment. Sees all procurement. |
| Carol | `Group-C` | Sees only Sedgman rows in financials, incidents, equipment. Sees all procurement. |
...
RETURN
    ...
    WHERE
        ...
        (@division = 'CPB Contractors' AND IS_MEMBER('Group-A') = 1)
        OR (@division = 'Thiess' AND IS_MEMBER('Group-B') = 1)
        OR (@division = 'Sedgman' AND IS_MEMBER('Group-C') = 1)
...
ADD FILTER PREDICATE security.fn_division_filter(division)
ON cimic.equipment.equipment_telemetry
WITH (STATE = ON);
GO
...
ADD FILTER PREDICATE security.fn_division_filter(division)
ON cimic.procurement.materials
WITH (STATE = ON);
GO
```
...
If you're in `Group-Executives`, you'll see all rows. Ask a colleague in `Group-A` to run the same query — they should only see CPB Contractors rows.
...
SQL Analytics Endpoint
    ...
    │
    ▼ returns only CPB Contractors rows
Data Agent formats response
    │
    ▼
Alice sees only CPB Contractors data ✅
```
...
1. `CIMIC Finance Agent` — only has access to `financials` table
2. `CIMIC Safety Agent` — only has access to `incidents` table
...
|-------|--------|---------|
| Application (client) ID: `5319dcfd-60d3-4bce-9d26-7e9a6dd81503` | Azure App Registration | Databricks Account Console + Fabric connection |
| Directory (tenant) ID: `f7bfcd96-5c56-4a99-8851-b2f678d69f15` | Azure App Registration | Fabric connection |
...
| **Some tables mirrored but others show error** | Those tables have RLS/CLM applied in Databricks | **Most likely cause for CIMIC workshop.** The `financials`, `equipment_telemetry`, and `incidents` tables have row filters from Module 3. Drop the filters or create unfiltered views — see Step C3 warning box for SQL commands. |
...
Ask these in the Databricks Genie Space. Vinoth should only see CPB Contractors rows.

**Positive tests (should return data):**

1. **"Show me all projects"**
   → ✅ Should return ONLY CPB Contractors projects (not Thiess, Sedgman, or Pacific Partnerships)

2. **"What is the total budget across all my projects?"**
   → ✅ Should show CPB Contractors total only

3. **"List all safety incidents with severity Critical or Major"**
   → ✅ Should show only CPB Contractors incidents

4. **"Which equipment has engine temperature above 100°C?"**
   → ✅ Should show only CPB Contractors equipment

5. **"Show me all projects in Red status with their CPI and SPI"**
   → ✅ Should show only CPB Contractors red projects

**Negative tests (should NOT show other divisions):**

6. **"Show me Thiess projects"**
   → ❌ Should return zero rows (Vinoth can't see Thiess data)

7. **"Compare all divisions side by side"**
   → ✅ Should only show CPB Contractors (filter prevents seeing other divisions)

8. **"What is the total budget for Sedgman?"**
...
10. **"Give me a risk summary — red projects, critical incidents, and overdue equipment"**
    → ✅ Should show only CPB Contractors data across all three domains

### Fabric Data Agent — RLS Test Questions (as Vinoth / Group-A)

Ask these in the Fabric Data Agent. If OneLake Security roles are set up correctly, Vinoth should only see CPB Contractors rows from the mirrored DB.

**Positive tests (mirrored DB — should be filtered):**

1. **"Show me all projects from the mirrored database"**
   → ✅ Should return ONLY CPB Contractors projects from `financials_f`

2. **"How many safety incidents are there by severity?"**
   → ✅ Should show only CPB Contractors incidents from `incidents_f`

3. **"What equipment needs maintenance?"**
   → ✅ Should show only CPB Contractors equipment from `equipment_telemetry_f`

4. **"What is the total budget and actual cost for my projects?"**
   → ✅ Should show CPB Contractors totals only

5. **"Show me all Critical incidents that are still Open"**
   → ✅ Should show only CPB Contractors critical incidents

**Negative tests (should NOT show other divisions):**

6. **"Show me Thiess project budgets"**
   → ❌ Should return zero rows from mirrored DB

7. **"Compare divisions on safety incidents"**
   → ✅ Should only show CPB Contractors (OLS prevents other divisions)
...
|----------|-------------------|----------------------|
| "Show me all projects" | CPB Contractors only | All 4 divisions |
| "Total budget" | CPB total | Full portfolio total |
| "Safety incidents" | CPB only | All divisions |
| "Thiess projects" | Zero rows | Thiess projects shown |
...
-- Count rows per division (should only show CPB Contractors)
SELECT division, COUNT(*) AS row_count
FROM projects.financials_f
GROUP BY division;
-- Expected: only one row — 'CPB Contractors'
...
-- Expected: only CPB Contractors rows
```
