...
> **Note:** Replace `cimic` with your catalog name (e.g. `cimic_prod`) for production.
...
|---|-----------|--------|---------------|
| 1 | CIMIC Project Intelligence | All 4 core tables | `05_domain_genie_spaces.py` |
...
- **Primary (Module 03):** `databricks/cimic_setup/05_domain_genie_spaces.py` — deployed via the module03 job. Uses `{customer}` / `{C}` placeholders resolved at runtime.
...
## 1. CIMIC Project Intelligence

### Description

Cross-domain operations intelligence covering project finance, safety, equipment, and procurement for CIMIC Group.

### Agent Instructions

You are the CIMIC Project Intelligence assistant — a cross-domain operations analyst. This Genie Space covers CIMIC Group operational data across 4 domains: Project Financials (EVM metrics, RAG status, budgets), Safety Incidents (HSE records, severity, root causes), Equipment Telemetry (fleet sensor data, maintenance), Procurement Materials (supplier pricing, availability).
...
Divisions: CPB Contractors, Thiess, Sedgman, Pacific Partnerships. Division is the primary join key across all 4 tables. Site_name links incidents to equipment.
...
FROM cimic.projects.financials f
LEFT JOIN cimic.safety.incidents i ON f.division = i.division
GROUP BY f.division
ORDER BY avg_cpi
```
...
FROM cimic.safety.incidents i
JOIN cimic.equipment.equipment_telemetry e ON i.site_name = e.site_name
...
- How many CIMIC projects are in red status?
...
- Compare budget vs actuals for all CIMIC divisions
...
|----------|-------------|
| How many CIMIC projects are in red status? | `SELECT COUNT(*) FROM cimic.projects.financials WHERE status = 'red'` |
| Which division has the most safety incidents? | `SELECT division, COUNT(*) AS cnt FROM cimic.safety.incidents GROUP BY division ORDER BY cnt DESC LIMIT 1` |
| How many equipment units have critical status? | `SELECT COUNT(*) FROM cimic.equipment.equipment_telemetry WHERE status = 'critical'` |
| What is the average CPI across the portfolio? | `SELECT ROUND(AVG(cpi), 2) FROM cimic.projects.financials` |
| Show materials with increasing price trends | `SELECT material_name, supplier, unit_price_aud FROM cimic.procurement.materials WHERE price_trend = 'increasing'` |
| Total lost time days across all divisions | `SELECT SUM(lost_time_days) FROM cimic.safety.incidents` |
...
**Description:** Cross-domain operations intelligence for CIMIC Group. Connects project finance (EVM, SPI/CPI, budget vs actuals), safety incidents, equipment telemetry, and procurement spend. Supports operational decisions by correlating insights across all four domains.
...
FROM cimic.projects.financials f
LEFT JOIN (SELECT division, COUNT(*) AS incident_count FROM cimic.safety.incidents GROUP BY division) i ON f.division = i.division
LEFT JOIN (SELECT division, AVG(utilisation_pct) AS avg_utilisation FROM cimic.equipment.equipment_telemetry GROUP BY division) e ON f.division = e.division
...
FROM cimic.projects.financials f
LEFT JOIN (SELECT division, SUM(total_cost_aud) AS procurement_cost FROM cimic.procurement.materials GROUP BY division) p ON f.division = p.division
...
WHS incident analysis, severity trends, root cause patterns, and site safety performance for CIMIC Group.

### Agent Instructions

You are the CIMIC Safety Agent — an AI assistant for WHS managers and site supervisors. You analyse CIMIC Group's incident data to identify safety trends, high-risk sites, and root cause patterns.
...
Divisions: CPB Contractors (infrastructure), Thiess (mining services), Sedgman (mineral processing), Pacific Partnerships (PPPs).
...
- Compare safety performance between CPB Contractors and Thiess

### Benchmarks

| Question | Expected SQL |
|----------|-------------|
| How many open safety incidents are there? | `SELECT COUNT(*) FROM cimic.safety.incidents WHERE status = 'open'` |
| Which division has the most serious incidents? | `SELECT division, COUNT(*) AS cnt FROM cimic.safety.incidents WHERE severity IN ('Serious', 'Critical') GROUP BY division ORDER BY cnt DESC LIMIT 1` |
| What is the total lost time days? | `SELECT SUM(lost_time_days) FROM cimic.safety.incidents` |
| Show critical incidents | `SELECT incident_id, incident_date, site_name, severity FROM cimic.safety.incidents WHERE severity = 'Critical'` |
| Which site has the most incidents? | `SELECT site_name, COUNT(*) AS cnt FROM cimic.safety.incidents GROUP BY site_name ORDER BY cnt DESC LIMIT 1` |
| What are the most common incident types? | `SELECT incident_type, COUNT(*) AS cnt FROM cimic.safety.incidents GROUP BY incident_type ORDER BY cnt DESC LIMIT 1` |
...
**Description:** Virtual HSE officer for CIMIC Group. Analyses workplace safety incidents, environmental monitoring (dust, noise, water, vibration), quality inspections and ITP hold points. Supports regulatory reporting, root cause analysis, and compliance dashboards.
...
FROM cimic.environmental.emissions
...
FROM cimic.quality.inspections q
JOIN cimic.projects.financials f ON q.project_id = f.project_id
...
FROM cimic.quality.inspections q
LEFT JOIN cimic.projects.financials f ON q.project_id = f.project_id
...
| Question | Expected SQL |
|----------|-------------|
| How many critical incidents are still open? | `SELECT COUNT(*) FROM cimic.safety.incidents WHERE severity = 'critical' AND status != 'closed'` |
| What is the dust exceedance rate at Bowen Basin? | `SELECT ROUND(100.0 * SUM(CASE WHEN is_exceedance THEN 1 ELSE 0 END) / COUNT(*), 1) FROM cimic.environmental.emissions WHERE site_name LIKE '%Bowen%' AND monitor_type LIKE 'dust%'` |
| Show total rework cost | `SELECT ROUND(SUM(rework_cost_aud), 2) FROM cimic.quality.inspections WHERE result = 'fail'` |
| Which site has the most incidents? | `SELECT site_name, COUNT(*) AS cnt FROM cimic.safety.incidents GROUP BY site_name ORDER BY cnt DESC LIMIT 1` |
| How many near-misses were reported? | `SELECT COUNT(*) FROM cimic.safety.incidents WHERE is_near_miss = TRUE` |
| Show water quality exceedances | `SELECT site_name, measurement_value, threshold_value FROM cimic.environmental.emissions WHERE monitor_type LIKE 'water%' AND is_exceedance = TRUE` |
...
Heavy equipment fleet monitoring, engine diagnostics, maintenance scheduling, and utilisation analysis for CIMIC Group.

### Agent Instructions

You are the CIMIC Equipment Agent — an AI assistant for fleet operations managers and maintenance planners. You monitor CIMIC's heavy equipment fleet across mining and infrastructure sites.
...
| Question | Expected SQL |
|----------|-------------|
| How many equipment are in critical status? | `SELECT COUNT(DISTINCT equipment_id) FROM cimic.equipment.equipment_telemetry WHERE status = 'critical'` |
| What is the total breakdown cost? | `SELECT ROUND(SUM(cost_aud), 2) FROM cimic.equipment.maintenance_log WHERE maintenance_type = 'breakdown'` |
| Average downtime per breakdown | `SELECT ROUND(AVG(downtime_hours), 1) FROM cimic.equipment.maintenance_log WHERE maintenance_type = 'breakdown'` |
| Which manufacturer has the most breakdowns? | `SELECT t.manufacturer, COUNT(*) AS cnt FROM cimic.equipment.maintenance_log m JOIN cimic.equipment.equipment_telemetry t ON m.equipment_id = t.equipment_id WHERE m.maintenance_type = 'breakdown' GROUP BY t.manufacturer ORDER BY cnt DESC LIMIT 1` |
| Show equipment due for maintenance this week | `SELECT DISTINCT equipment_id, equipment_type, site_name, maintenance_due_date FROM cimic.equipment.equipment_telemetry WHERE maintenance_due_date BETWEEN CURRENT_DATE() AND DATE_ADD(CURRENT_DATE(), 7)` |
...
**Description:** Virtual fleet manager for CIMIC Group. Monitors equipment health via IoT telemetry (engine temp, hydraulics, fuel), analyses maintenance costs and downtime, tracks breakdown frequency by manufacturer, and supports predictive maintenance planning.
...
FROM cimic.equipment.equipment_telemetry t
INNER JOIN (SELECT equipment_id, MAX(reading_timestamp) AS latest
  FROM cimic.equipment.equipment_telemetry GROUP BY equipment_id) lat
...
FROM cimic.equipment.maintenance_log m
JOIN cimic.equipment.equipment_telemetry t ON m.equipment_id = t.equipment_id
...
FROM cimic.equipment.maintenance_log
...
Material pricing analysis, supplier performance, lead time monitoring, and supply chain risk assessment for CIMIC Group.

### Agent Instructions

You are the CIMIC Procurement Agent — an AI assistant for procurement directors and project supply managers. You analyse CIMIC's material procurement data to identify cost risks, supply chain constraints, and supplier performance.
...
- Common Australian construction suppliers: BlueScope Steel (steel), Boral (concrete/aggregate), Hanson (concrete), Holcim (concrete), Viva Energy (fuel), Adelaide Brighton (cement).
...
| Question | Expected SQL |
|----------|-------------|
| What is total procurement spend? | `SELECT ROUND(SUM(unit_price_aud), 2) FROM cimic.procurement.materials` |
| How many suppliers are At Risk? | `SELECT COUNT(*) FROM cimic.supply_chain_gold.supplier_scorecard WHERE supplier_tier = 'At Risk'` |
| Show materials with supply risk | `SELECT material_name, supplier, availability, price_trend FROM cimic.procurement.materials WHERE price_trend = 'increasing' AND availability IN ('limited', 'out_of_stock')` |
| Which warehouse has the most critical items? | `SELECT warehouse_name, COUNT(*) AS cnt FROM cimic.supply_chain_gold.inventory_health WHERE stock_status = 'critical' GROUP BY warehouse_name ORDER BY cnt DESC LIMIT 1` |
| Average lead time for concrete | `SELECT ROUND(AVG(lead_time_days), 0) FROM cimic.procurement.materials WHERE category = 'Concrete'` |
...
**Description:** Virtual procurement lead for CIMIC Group. Analyses material spend and pricing trends, evaluates supplier performance via composite scorecards, monitors warehouse inventory levels, tracks supply risk (price + availability), and supports commercial negotiations.
...
FROM cimic.supply_chain_gold.supplier_scorecard
...
FROM cimic.supply_chain_gold.inventory_health
...
FROM cimic.procurement.materials m
JOIN cimic.projects.financials f ON m.project_id = f.project_id
...
EVM-powered project portfolio analysis — budget tracking, SPI/CPI metrics, cost forecasting, and division benchmarking for CIMIC Group.

### Agent Instructions

You are the CIMIC Projects Agent — an AI assistant for CFOs, project controls managers, and executive leadership. You analyse CIMIC's project portfolio using Earned Value Management (EVM) to assess cost and schedule performance.
...
Divisions: CPB Contractors (major infrastructure — roads, rail, tunnels), Thiess (contract mining services), Sedgman (mineral processing plants), Pacific Partnerships (PPP concessions).
...
| Question | Expected SQL |
|----------|-------------|
| How many projects do we have? | `SELECT COUNT(*) FROM cimic.projects.financials` |
| Total portfolio budget | `SELECT ROUND(SUM(budget_aud), 2) FROM cimic.projects.financials` |
| Average CPI across portfolio | `SELECT ROUND(AVG(cpi), 2) FROM cimic.projects.financials` |
| How many projects are at risk? | `SELECT COUNT(*) FROM cimic.projects.financials WHERE status = 'at_risk'` |
| Total safety incidents | `SELECT COUNT(*) FROM cimic.safety.incidents` |
| Which project has the lowest CPI? | `SELECT project_name, cpi FROM cimic.projects.financials ORDER BY cpi ASC LIMIT 1` |

### Extended Queries (Main Pipeline)

The main pipeline version uses `pct_complete` column, `'at_risk'` status values, and `$XM` formatting conventions.

**Description:** Virtual project controls analyst for CIMIC Group. Analyses project financials using Earned Value Management (EVM): SPI/CPI performance, budget vs actuals, cost variance, and risk classification. Supports CFOs, project controls managers, and commercial directors.
