...
> **Scope:** Complete reference for Databricks permissions — covering workspace access, Unity Catalog data permissions, Genie Space ACLs, and how they all layer together. Written for the CIMIC workshop context.
...
10. [Practical Setup: CIMIC Workshop](#10-practical-setup-cimic-workshop)
...
Metastore (account-level, one per region)
  └── Catalog (e.g., cimic)
        └── Schema (e.g., cimic.projects)
              ├── Table (e.g., cimic.projects.financials)
              ├── View
              ├── Function (e.g., cimic.security.division_filter)
              └── Volume
```
...
|-----------|---------------|-------------|
| **USE CATALOG** | Navigate into the catalog; see its schemas | `GRANT USE CATALOG ON CATALOG cimic TO 'user@company.com'` |
| **CREATE SCHEMA** | Create new schemas in the catalog | `GRANT CREATE SCHEMA ON CATALOG cimic TO 'data_engineers'` |
| **ALL PRIVILEGES** | Everything on the catalog | `GRANT ALL PRIVILEGES ON CATALOG cimic TO 'admin_group'` |

#### Schema-Level

| Permission | What It Allows | SQL Example |
|-----------|---------------|-------------|
| **USE SCHEMA** | Navigate into the schema; see its tables | `GRANT USE SCHEMA ON SCHEMA cimic.projects TO 'user@company.com'` |
| **SELECT** | Read data from all tables in the schema | `GRANT SELECT ON SCHEMA cimic.projects TO 'analyst_group'` |
| **MODIFY** | INSERT, UPDATE, DELETE on tables | `GRANT MODIFY ON SCHEMA cimic.projects TO 'data_engineers'` |
| **CREATE TABLE** | Create new tables in the schema | `GRANT CREATE TABLE ON SCHEMA cimic.projects TO 'data_engineers'` |
| **CREATE FUNCTION** | Create UDFs in the schema | `GRANT CREATE FUNCTION ON SCHEMA cimic.security TO 'admin_group'` |
| **EXTERNAL USE SCHEMA** | Read Delta files directly from storage (ADLS) — **required for Fabric mirroring** | `GRANT EXTERNAL USE SCHEMA ON SCHEMA cimic.projects TO 'sp-client-id'` |
| **ALL PRIVILEGES** | Everything on the schema | `GRANT ALL PRIVILEGES ON SCHEMA cimic.projects TO 'admin_group'` |

#### Table-Level

| Permission | What It Allows | SQL Example |
|-----------|---------------|-------------|
| **SELECT** | Read data from this specific table | `GRANT SELECT ON TABLE cimic.projects.financials TO 'user@company.com'` |
| **MODIFY** | INSERT, UPDATE, DELETE on this table | `GRANT MODIFY ON TABLE cimic.projects.financials TO 'data_engineers'` |
...
To query `cimic.projects.financials`, a user needs exactly:

```sql
GRANT USE CATALOG ON CATALOG cimic TO `user@company.com`;
GRANT USE SCHEMA ON SCHEMA cimic.projects TO `user@company.com`;
GRANT SELECT ON TABLE cimic.projects.financials TO `user@company.com`;
```
...
GRANT USE CATALOG ON CATALOG cimic TO `user@company.com`;
GRANT USE SCHEMA ON SCHEMA cimic.projects TO `user@company.com`;
GRANT SELECT ON SCHEMA cimic.projects TO `user@company.com`;
```
...
```sql
GRANT USE CATALOG ON CATALOG cimic TO `user@company.com`;
GRANT USE SCHEMA ON CATALOG cimic TO `user@company.com`;
GRANT SELECT ON CATALOG cimic TO `user@company.com`;
```
...
### Example: Alice wants to ask a question in the CIMIC Genie Space
...
Alice tries to ask: "Show me red-status CPB projects"
    ...
    └── YES → continue
          ...
          └── CAN QUERY → continue
                │
                ▼ Genie generates SQL:
                │  SELECT * FROM cimic.projects.financials WHERE status = 'red'
                │
                ▼ Layer 2: Does Alice have SELECT on cimic.projects.financials?
...
| 3 | Unity Catalog | **USE CATALOG** on `cimic` | SQL: `GRANT USE CATALOG ON CATALOG cimic TO ...` |
| 4 | Unity Catalog | **USE SCHEMA** on relevant schemas | SQL: `GRANT USE SCHEMA ON SCHEMA cimic.projects TO ...` |
| 5 | Unity Catalog | **SELECT** on relevant tables/schemas | SQL: `GRANT SELECT ON SCHEMA cimic.projects TO ...` |
...
### For the CIMIC Workshop

| User Type | Workspace Role | SQL Warehouse | Unity Catalog | Genie Space | Entra Group |
|-----------|:-----:|:-----:|:-----:|:-----:|:-----:|
| **Workshop presenter** | Admin | CAN MANAGE | ALL PRIVILEGES on `cimic` | CAN MANAGE | Group-Executives |
| **CPB analyst** | User | CAN USE | USE CATALOG + USE SCHEMA + SELECT on `cimic` | CAN QUERY | Group-A |
| **Thiess analyst** | User | CAN USE | USE CATALOG + USE SCHEMA + SELECT on `cimic` | CAN QUERY | Group-B |
| **Executive** | User | CAN USE | USE CATALOG + USE SCHEMA + SELECT on `cimic` | CAN QUERY | Group-Executives |
| **Genie Space curator** | User | CAN USE | USE CATALOG + USE SCHEMA + SELECT on `cimic` | CAN EDIT | (any division) |
...
| 2. Genie generates SQL | `SELECT * FROM cimic.projects.financials` |
...
## 10. Practical Setup: CIMIC Workshop

### Quick Setup Script for Workshop Participants

Run this in a Databricks SQL Editor as a workspace admin to grant a group read access to all CIMIC data:
...
GRANT USE CATALOG ON CATALOG cimic TO `CIMIC-Workshop-Participants`;
GRANT USE SCHEMA ON CATALOG cimic TO `CIMIC-Workshop-Participants`;
GRANT SELECT ON CATALOG cimic TO `CIMIC-Workshop-Participants`;
```

Then share the Genie Space:
1. Open the Genie Space → **Share** → add `CIMIC-Workshop-Participants` with **CAN QUERY**
...
- Bob (Thiess group) queries through Genie → sees only Thiess data
...
| Genie says "Permission denied" on query | Unity Catalog | Missing SELECT on the table | `GRANT SELECT ON SCHEMA cimic.projects TO ...` |
| Genie says "Cannot resolve catalog" | Unity Catalog | Missing USE CATALOG | `GRANT USE CATALOG ON CATALOG cimic TO ...` |
...
| Bob | Thiess Division | Thiess projects only, budget rounded |
