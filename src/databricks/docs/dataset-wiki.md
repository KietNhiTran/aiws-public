# Contoso Group -- AI Workshop Dataset Wiki

## The Story

Contoso Group is Australia's largest infrastructure and construction conglomerate, operating
across mining services, civil engineering, commercial building, and public infrastructure.
With annual revenues exceeding AUD 14 billion and projects spanning every state, Contoso faces
a universal challenge: **how do you govern, monitor, and optimise thousands of moving parts
across a portfolio of megaprojects?**

This dataset models a realistic slice of Contoso's operational world -- seven active construction
...
answer questions a Contoso executive, project director, or AI agent would ask:
...
All data lives in the Unity Catalog: **`contoso_dev`**
...
| division | STRING | Contoso division: Division-Alpha, Division-Beta, UGL, Division-Gamma, EIC Activities |
...
processing logistics data for the same Contoso sites.
...
### Imagine you are the Contoso Chief Operating Officer...
...
All data references these Contoso project sites:

| Site Name | State | Division | Project Type | Example Project |
|-----------|-------|----------|-------------|-----------------|
| Sydney Metro West | NSW | Division-Alpha | rail | Underground rail tunnel and station construction |
| WestConnex M4-M5 | NSW | Division-Alpha | highway | Major motorway tunnel link in Sydney's inner west |
| Cross River Rail | QLD | Division-Alpha | rail | Brisbane underground rail crossing |
| Bowen Basin Mine | QLD | Division-Beta | mining | Coal mine expansion and processing |
| Pilbara Iron Ore | WA | Division-Beta | mining | Iron ore mine infrastructure |
| Melbourne Metro | VIC | Division-Alpha | rail | Metro tunnel and station box construction |
| Snowy 2.0 | NSW | Division-Alpha | water | Pumped hydro expansion in the Snowy Mountains |
...
SELECT * FROM contoso_dev.projects.v_project_risk_matrix
ORDER BY risk_score DESC;

-- Safety KPIs per site
SELECT * FROM contoso_dev.safety.v_safety_summary;

-- Supplier scorecard (gold layer)
SELECT * FROM contoso_dev.supply_chain_gold.supplier_scorecard
...
FROM contoso_dev.projects.v_project_risk_matrix r
...
```
  ...
  +-------+  +-------+  +--------+  +--------+  +--------+  +--------+
      ...
      +-----------|-----------|-----------|-----------|-----------|-----+
                  |           |           |           |           |
              Unity Catalog: contoso_dev
              (9 manufacturing tables + views + MVs)
```

### Space Details

#### 1. Contoso Project Intelligence (All-in-One)
...
#### 2. Contoso Safety and Compliance Agent
...
#### 3. Contoso Equipment and Fleet Agent
...
#### 4. Contoso Procurement and Supply Chain Agent
...
#### 5. Contoso Workforce and Labour Agent
...
#### 6. Contoso Executive Portfolio Agent
...
- **Contoso Manufacturing - Initial Setup**: Schema creation, data generation, advanced
  objects, Genie Space creation, and domain agent deployment
- **Contoso Supply Chain - Medallion Pipeline**: Source simulation, bronze ingestion,
...
databricks bundle run contoso_data_setup -t dev
...
This Databricks deployment is one half of the Contoso AI Workshop. The other half is
