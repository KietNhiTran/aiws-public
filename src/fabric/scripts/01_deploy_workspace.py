...
Creates all Fabric workspace items for the Data Agent multi-source demo:
  ...
  - Semantic Model (Contoso_KPI_Model)
  - Mirrored Databricks Catalog
  - 5 Data Agents (Contoso Project Intelligence, Projects, Safety, Equipment, Procurement)
...
Usage:
  ...
  python fabric/scripts/01_deploy_workspace.py --workspace "contoso-ws-dev" --capacity "REPLACE_WITH_CAPACITY_NAME"
...
# ═══════════════════════════════════════════════════════════════════════
DEFAULTS = {
    "workspace_name": os.environ.get("FABRIC_WORKSPACE", "contoso-aiws-dev"),
    "capacity_name": os.environ.get("FABRIC_CAPACITY", "REPLACE_WITH_CAPACITY_NAME"),
    "dbx_host": os.environ.get("DBX_HOST", "REPLACE_WITH_DATABRICKS_HOST"),
    "dbx_catalog": os.environ.get("DBX_CATALOG", "contoso"),
}
...
AGENT_FILE_MAP = {
    "Contoso Project Intelligence": "project-intelligence",
...
AGENTS = [
    {
        ...
        "instructions": (
            "You are a cross-domain operations intelligence assistant for Contoso Group.\n\n"
    ...
    },
    {
        ...
        "instructions": (
            "You are a project finance analyst for Contoso Group.\n\n"
    ...
    },
    {
        ...
        "instructions": (
            "You are an HSE (Health Safety Environment) analyst for Contoso Group.\n\n"
    ...
    },
    {
        ...
        "instructions": (
            "You are a fleet operations analyst for Contoso Group.\n\n"
    ...
    },
    {
        ...
        "instructions": (
            "You are a procurement analyst for Contoso Group.\n\n"
...
def build_semantic_model_tmsl(lakehouse_name):
    """Build a TMSL model.bim for Direct Lake over the Contoso lakehouse."""
    return {
        "compatibilityLevel": 1604,
        "model": {
            "name": "Contoso_KPI_Model",
            "culture": "en-AU",
            "tables": [
                {
                    "name": "ProjectKPIs",
            ...
            "annotations": [
                {"name": "Description", "value": "Contoso KPI summary model — Direct Lake over contoso_lakehouse for executive dashboard agent"}
            ]
        }
    }
...
# To reset: delete contoso_sqldb in portal → re-run this script → paste populate_sql_database.sql
...
# ═══════════════════════════════════════════════════════════════════════
def main():
    parser = argparse.ArgumentParser(description="Deploy Fabric workspace for Contoso AI Workshop")
    ...
    sm_id, sm_created = create_item(token, ws_id, "Contoso_KPI_Model", "SemanticModel", definition=sm_def, dry_run=args.dry_run)
    (summary["created"] if sm_created else summary["skipped"]).append("SemanticModel: Contoso_KPI_Model")
    ...
    if args.dry_run:
        ...
        config = {
            ...
            "items": {
                ...
                "semantic_model": {"name": "Contoso_KPI_Model", "id": sm_id},
    ...
    print(f"  Semantic:   Contoso_KPI_Model ({sm_id or 'dry-run'})")
