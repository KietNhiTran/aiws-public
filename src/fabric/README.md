...
```
┌─────────────────────────────────────────────────────────────────┐
│  Fabric Workspace: contoso-ws-dev                                 │
│                                                                 │
│  ┌──────────────┐  ┌──────────────┐  ┌───────────────────────┐ │
│  │  Lakehouse   │  │ SQL Database │  │ Mirrored Databricks   │ │
│  │  contoso_      │  │ contoso_sqldb  │  │ Catalog (contoso)       │ │
│  │  lakehouse   │  │              │  │                       │ │
│  │ ─────────────│  │ ─────────────│  │ projects.financials   │ │
...
│  ┌──────┴─────────────────┴──────────────────────┴───────────┐ │
│  │                    5 Data Agents                           │ │
│  │  • Contoso Project Intelligence → ALL sources (mirrored+LH+SQL+model) │ │
...
1. **Azure CLI** logged in to the Contoso tenant:
   ```bash
   az login --tenant <YOUR_TENANT_ID>
   ```

2. **Fabric capacity** provisioned (default: `REPLACE_WITH_CAPACITY_NAME`)
...
# Step 2: Populate SQL Database with Contoso summary data
...
| `scripts/02_populate_sql_db.py` | Populates SQL Database with Contoso data (division_summary, monthly_kpis, manufacturing_kpis, supplier_scorecard) |
...
| `notebooks/populate_lakehouse.ipynb` | PySpark notebook — upload to Fabric and run against contoso_lakehouse |
...
Ask the **Contoso Project Intelligence** agent: *"Give me a portfolio health summary with safety metrics and fleet status"*
→ Queries `semantic_model:Contoso_KPI_Model` + `lakehouse:contoso_lakehouse` + `sql_database:contoso_sqldb`
