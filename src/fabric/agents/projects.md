...
```
You are a project finance analyst for Contoso Group, Australia's largest infrastructure and mining services company. You help project controllers, finance managers, and divisional leaders analyse project financial performance using Earned Value Management (EVM) methodology.
...
- Division-Alpha — infrastructure and building projects
- Division-Beta — mining services
- Division-Gamma — mineral processing
...
```
Live-mirrored project financial data from Databricks Unity Catalog. Contains individual project records with full Earned Value Management (EVM) metrics: budget, actual cost, earned value, planned value, SPI, CPI, cost variance, and RAG status per project per reporting period. Covers all four Contoso divisions (Division-Alpha, Division-Beta, Division-Gamma, Division-Delta) across all Australian states. Use this source for granular project-level analysis, EVM calculations, project manager performance reviews, portfolio drilldowns by division/state/type, and identifying at-risk projects. Table: financials_f — one row per project per reporting period.
```

### Source: contoso_lakehouse — projectkpis — Description

```
Pre-aggregated project financial KPIs from the Contoso lakehouse. Provides division-level portfolio summaries — total budget, total actual cost, average CPI, average SPI, project count, and red project count per division. Use this source for fast divisional comparisons and portfolio health dashboards without needing to aggregate individual project records. One row per division. Table: projectkpis.
```

### Source: contoso_sqldb — monthly_kpis — Description

```
Monthly cross-functional KPI time series from the Contoso SQL database. Tracks budget, actual cost, safety incidents, near misses, lost time days, inspection pass/fail rates, and environmental exceedances per division per month over 12 months. Use this source for monthly trend analysis, period-over-period comparisons, and identifying divisions with deteriorating financial or safety performance over time. Table: monthly_kpis — 48 rows (12 months × 4 divisions).
```
...
```
TABLE: contoso_dbx_org.projects.financials_f
...
### Source: contoso_lakehouse — projectkpis — Instructions

```
TABLE: contoso_lakehouse.dbo.projectkpis
...
### Source: contoso_sqldb — monthly_kpis — Instructions

```
TABLE: contoso_sqldb.dbo.monthly_kpis
...
6. "What is the Estimate at Completion (EAC) for all Division-Beta mining projects?"
7. "Which project managers have more than one project in Amber or Red status?"
8. "How does Division-Alpha' portfolio perform by project type?"
...
15. "Give me a complete financial health report for Division-Beta using both granular project data and the aggregated KPIs"
...
FROM contoso_dbx_org.projects.financials_f
WHERE reporting_period = (SELECT MAX(reporting_period) FROM contoso_dbx_org.projects.financials_f)
GROUP BY division
...
FROM contoso_dbx_org.projects.financials_f
WHERE status = 'Red'
  AND reporting_period = (SELECT MAX(reporting_period) FROM contoso_dbx_org.projects.financials_f)
...
```sql
-- Question: How does Division-Alpha' portfolio perform by project type?
SELECT
    project_type,
    COUNT(*) AS project_count,
    SUM(budget_aud) AS total_budget,
